...
function Details2(props) {
  ...
  var settings = {
    ...
    let {id}=useParams();
  let item=props.ebook.find((a)=>a.id==id);
    ...
    <div className='yaldetails'>
      ...
      <div className="detailmain">
        ...
        <div className="detailtext">
          <div className="detatexttit"><h1>{item.name}</h1></div>
          ...
          <div className="dettype"><h2>{`${item.type}, ${item.pages} səhifə`}</h2></div>
          <div className="detdate"><h4>{item.date}</h4></div>
          <div className="priceflex">
          <div className="detprice">{`$ ${Number(item.dicscount)?item.dicscount:item.price}`}</div>
          <div className="gozsox">{`${Number(item.dicscount)? `• ${(item.price-item.dicscount).toFixed(2)} $ endirim ilə`:item.dicscount}`}</div>
          </div>
          <div className="detbtns">
            <div className="detbtn1"><a ><h3>Böyük format</h3><h2>{` ${item.largep}`}</h2></a></div>
            <div className="detbtn1"><a ><h3>Audio format</h3><h2>{`${Number(item.audiop)? `$ ${item.audiop}`: item.audiop}`}</h2></a></div>
          </div>
          <div className="detlangu"><h3>{`${item.language} dilində/dillərində əldə edə bilərsiniz`}</h3></div>
      ...
      <div className="detabouts">
        <div className="detabt2"><h5>{item.about3}</h5></div>
      </div>
      <Mainbest/>
...
let t=(s)=>s;
export default connect(t)(Details2);